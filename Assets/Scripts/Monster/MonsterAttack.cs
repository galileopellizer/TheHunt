...
public class MonsterAttack : NetworkBehaviour
{
    ...
    private void Update()
    {
        ...
        int animIndex = animController != null ? animController.PickAttackIndex() : 0;
        animController?.TriggerAttack(animIndex);
        MonsterStealth.LocalMonsterStealth?.ResetStillTimer();
        TryAttackServerRpc(animIndex);
    }

    [ServerRpc]
    private void TryAttackServerRpc(int animIndex, ServerRpcParams rpcParams = default)
    {
        if (!IsServer) return;
        if (selfRole == null || selfRole.Role.Value != PlayerRole.Monster) return;
        if (selfHealth != null && selfHealth.IsDead.Value) return;

        TriggerAttackAnimClientRpc(animIndex);

        // Reveal on any attack attempt (hit or miss)
    ...
    [ClientRpc]
    private void TriggerAttackAnimClientRpc(int animIndex)
    {
